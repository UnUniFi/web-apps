...
describe('DecimalsPipe', () => {
  ...
  it('extracts the number from 1 decimal place to the entered digit', () => {
    expect(pipe.transform('123.456', 3)).toBe('.456');
  });

  it('returns all numbers after the decimal point, when input a number with more than its decimal point', () => {
    expect(pipe.transform('123.456', 4)).toBe('.456');
  });

  it('returns dot string, when input a number 0', () => {
    expect(pipe.transform('123.456', 0)).toBe('.');
  });

  it('returns empty string, when input a integer', () => {
