...
export class BlockComponent implements OnInit {
  ...
  constructor(private route: ActivatedRoute, private cosmosSDK: CosmosSDKService) {
    ...
    this.block$ = combineLatest([this.cosmosSDK.sdk$, this.blockHeight$]).pipe(
      mergeMap(([sdk, height]) =>
        conmosclient.rest.tendermint.getBlockByHeight(sdk.rest, BigInt(height)).then((res) => res.data),
      ),
    );
    ...
    this.latestBlockHeight$ = this.cosmosSDK.sdk$.pipe(
      mergeMap((sdk) => conmosclient.rest.tendermint.getLatestBlock(sdk.rest).then((res) => res.data)),
